...
class ObjectStoreTestCase(TestCase):
    ...
    def test_upload(self, mocked_get_container_path, mocked_get_connection):
        ...
        file = ("mock", "file")
        document = Document(
            filename="doc_file_name.pdf", type=Document.DocumentType.Ontwerp
        )
        objstore.upload(file, document)
        connection.put_object.assert_called_with(
            "container/path/", "doc_file_name.pdf", file
        )
    ...
    def test_delete(self, mocked_get_container_path, mocked_get_connection):
        ...
        connection.delete_object.assert_called_with(
            "container/path/", "doc_file_name.pdf"
        )

    @mock.patch("storage.object_store.get_connection")
    @mock.patch("storage.object_store.ObjectStore.get_container_path")
    def test_delete_non_existing_file(
        self, mocked_get_container_path, mocked_get_connection
    ):
        """
        Test and assert that delete_object is called with the correct arguments
        """
        ...
        document = Document(
            id=1, filename="doc_file_name.pdf", type=Document.DocumentType.Ontwerp
        )
        ...
        self.assertIn(
            "INFO:storage.object_store:Failed to delete object for document id 1",
            logs.output,
        )
    ...
    def test_get_file_download(self, mocked_file, mocked_isfile, mocked_makedirs):
        ...
        objstore = ObjectStore(config="this is the config")
        with self.assertLogs(level="INFO") as logs:
            return_value = objstore.get_file(
                connection, container_name, path, object_name
            )

        mocked_makedirs.assert_called_with(
            f"{DOWNLOAD_DIR}{container_name}/{path}", exist_ok=True
        )
        self.assertNotIn(
            "INFO:storage.object_store:Using cached file: path_name_mock/object_name_mock",
            logs.output,
        )
        connection.get_object.assert_called_with(
            container_name, f"{path}/{object_name}"
        )
        mocked_file().write.assert_called_with("mocked_data")
        self.assertEqual(
            return_value, f"{DOWNLOAD_DIR}{container_name}/{path}/{object_name}"
        )
